...
namespace SplineTools {
    ...
    public class PoolObject : ScriptableObject {        // TODO also abstract. this is pretty specifically a MESH object
        ...
        public GameObject CreateSelfAndMeasureSize (Material newMaterial, Quaternion localRotation, out float linearSize) {
            var newGO = Instantiate(prefab, Vector3.zero, localRotation, null);
            if(newMaterial != null){
            ...
            if(col != null){
                ...
                var localDir = newGO.transform.InverseTransformVector(Vector3.forward).normalized;
    ...
    public class BSplineObjectEditor : Editor {
        ...
        public override void OnInspectorGUI () {
            ...
            if(useBoxCol && boxCols.Length != 1){
                Prefix();
                if(boxCols.Length < 1){
                    EditorGUILayout.LabelField(PoolObject.NO_BOXCOLLS_WARNING);
                }else{
                    EditorGUILayout.LabelField(PoolObject.TOO_MANY_BOXCOLLS_WARNING);
                }
